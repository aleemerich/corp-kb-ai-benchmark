...
A arquitetura do audit trail segue o padrão de microsserviços, permitindo escalabilidade independente e facilitando a manutenção. O sistema é composto por:
...
O audit trail é um dos pilares fundamentais do AIRich API Gateway, parte integrante do ecossistema de produtos da AIRich Tecnologia. Desde sua concepção, este componente foi projetado para atender empresas de diversos portes, desde startups até grandes corporações com operações em múltiplos países.
...
A implementação do audit trail utiliza tecnologias modernas e consolidadas no mercado:
...
O audit trail é monitorado 24/7 através de:
...
O fluxo típico de operação do audit trail segue as seguintes etapas:
...
A segurança do audit trail é tratada em múltiplas camadas:
