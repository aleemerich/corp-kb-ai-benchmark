...
O strong customer authentication é um dos pilares fundamentais do AIRich Pay, parte integrante do ecossistema de produtos da AIRich Tecnologia. Desde sua concepção, este componente foi projetado para atender empresas de diversos portes, desde startups até grandes corporações com operações em múltiplos países.
...
A segurança do strong customer authentication é tratada em múltiplas camadas:
...
A arquitetura do strong customer authentication segue o padrão de microsserviços, permitindo escalabilidade independente e facilitando a manutenção. O sistema é composto por:
...
graph TD
    ...
    B --> C[Serviço de Strong Customer Authentication]
    C --> D[Cache Redis]
    C --> E[Banco de Dados]
...
O fluxo típico de operação do strong customer authentication segue as seguintes etapas:
...
O strong customer authentication é monitorado 24/7 através de:
