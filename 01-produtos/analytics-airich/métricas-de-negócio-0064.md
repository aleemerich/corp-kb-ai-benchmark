...
A arquitetura do métricas de negócio segue o padrão de microsserviços, permitindo escalabilidade independente e facilitando a manutenção. O sistema é composto por:
...
graph TD
    ...
    B --> C[Serviço de Métricas de negócio]
    C --> D[Cache Redis]
    C --> E[Banco de Dados]
...
O métricas de negócio é monitorado 24/7 através de:
...
A implementação do métricas de negócio utiliza tecnologias modernas e consolidadas no mercado:
...
O métricas de negócio é um dos pilares fundamentais do AIRich Analytics, parte integrante do ecossistema de produtos da AIRich Tecnologia. Desde sua concepção, este componente foi projetado para atender empresas de diversos portes, desde startups até grandes corporações com operações em múltiplos países.
...
O fluxo típico de operação do métricas de negócio segue as seguintes etapas:
