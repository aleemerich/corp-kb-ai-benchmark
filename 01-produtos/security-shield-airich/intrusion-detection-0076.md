...
> Documento detalhado do AIRich Security Shield - AIRich Tecnologia
...
O intrusion detection é um dos pilares fundamentais do AIRich Security Shield, parte integrante do ecossistema de produtos da AIRich Tecnologia. Desde sua concepção, este componente foi projetado para atender empresas de diversos portes, desde startups até grandes corporações com operações em múltiplos países.
...
A implementação do intrusion detection utiliza tecnologias modernas e consolidadas no mercado:
...
O fluxo típico de operação do intrusion detection segue as seguintes etapas:
...
O intrusion detection é monitorado 24/7 através de:
...
A arquitetura do intrusion detection segue o padrão de microsserviços, permitindo escalabilidade independente e facilitando a manutenção. O sistema é composto por:
...
graph TD
    ...
    B --> C[Serviço de Intrusion detection]
    C --> D[Cache Redis]
    C --> E[Banco de Dados]
...
A segurança do intrusion detection é tratada em múltiplas camadas:
